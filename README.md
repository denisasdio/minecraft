...
## Jalankan dengan Docker Desktop

Pastikan Docker Desktop sudah berjalan, lalu dari folder proyek jalankan:

```sh
docker compose up --build -d
```

Buka [http://localhost:8001](http://localhost:8001). Untuk melihat log, gunakan
`docker compose logs -f`; untuk menghentikannya, gunakan `docker compose down`.

Port dapat diganti tanpa mengubah berkas konfigurasi. Contoh untuk memakai port
3000:
...
```powershell
$env:APP_PORT=3000; docker compose up --build -d
```
